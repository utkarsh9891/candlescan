...
function runWindow(stockDataForWindow, { detectPatterns, detectLiquidityBox, computeRiskScore, MIN_CONFIDENCE, MAX_POSITIONS, POSITION_SIZE, CAPITAL, SKIP_FIRST_BARS, MAX_TOTAL_TRADES, indexDirection, marginEnabled, marginMap, sectorData, vixReg, flowClass, newsMap, pessimisticFills, useFlow, regimeAwareStops, sizeTiers, maxConsecLosses }) {
  ...
  for (let barIdx = 0; barIdx < maxBars; barIdx++) {
    ...
    for (let p = openPositions.length - 1; p >= 0; p--) {
      ...
      // When the risk engine returned a tranche ladder, each leg has its
      // own target px, full position shares×pct slice, and a shared SL +
      // EOD fallback. Process each open tranche on every bar; when all
      // are closed we record one trade row whose exit/exitReason summarize
      // the ladder.
      if (pos.tranches && pos.tranches.length) {
        ...
        for (const t of pos.tranches) {
          ...
          if (t.state !== 'open') t.exitTime = formatTime(bar.t);
        }

        const allClosed = pos.tranches.every(t => t.state !== 'open');
        if (allClosed) {
          // Aggregate gross + tx cost across all tranches; weighted exit px.
          let grossPnl = 0;
          let totalShares = 0;
          let weightedExit = 0;
          let entryNotional = 0;
          ...
          const txCost = (entryNotional + exitNotional) * TX_COST_PCT;
          ...
          // exitReason summarizes the ladder (T1:tgt|T2:tgt|T3:eod etc.)
          const reasonStr = pos.tranches
            .map(t => `${t.name}:${t.state.slice(0, 3)}`).join('|');
          const lastExitTime = pos.tranches
            .map(t => t.exitTime).filter(Boolean).pop() || formatTime(bar.t);
    ...
    for (const c of candidates) {
      ...
      // Tranche-aware entry (Trend Continuation Pullback v11): the risk
      // engine returns a `tranches` ladder rebased to the *raw* entry
      // (pre-slippage). Reanchor each tranche's target to the actual
      // post-slip fill so the +0.2%/+0.4%/+0.4% percentages still hold.
      let tranchesForPos = null;
      if (c.risk.tranches && c.risk.tranches.length) {
        tranchesForPos = c.risk.tranches.map(t => {
          // Reanchor target % to post-slippage entry. We re-derive
          // targetPct from (target - rawEntry)/rawEntry for fidelity, then
          // rebuild the absolute target on entryPrice.
          const targetPct = (t.target - c.risk.entry) / c.risk.entry;
          const target = c.risk.direction === 'long'
            ? entryPrice * (1 + targetPct)
            : entryPrice * (1 + targetPct); // SHORT: targetPct is negative here
