...
function rupees(n) {
  ...
  const s = String(abs);
  // Crude Indian grouping for display only
...
function money(n) {
  // Approximate "Rs X,000" for novice-friendly rounding to nearest 100.
  if (typeof n !== 'number' || !isFinite(n)) return '—';
...
/**
 * Compute trade sizing the novice user sees.
 * Uses the CLAUDE.md standard: 3L capital → 15L exposure at 5x.
 */
function computeSizing(r, capital = DEFAULT_CAPITAL) {
 ...
 * Intentionally verbose: the target user can't parse "E:234 SL:232.8 T:236.4".
 */
function TradeNowCard({ r, capital, onTap, isNew }) {
  ...
  return (
    ...
    >
      {/* Top row: NEW badge + symbol + action chip */}
      ...
      <div style={{
        ...
        {sizing && (
          <Line label="Profit if it works" value={`≈ ${money(sizing.expectedGain)}`} color="#16a34a" muted />
        )}
        ...
        {sizing && (
          <Line label="Max loss if it fails" value={`≈ ${money(sizing.maxLoss)}`} color="#dc2626" muted />
        )}

        <div style={{ height: 1, background: '#f1f3f7', margin: '8px 0' }} />

        <Line
          label="Time out either way by"
          value="11:00 AM (30-min max hold)"
          muted
        />
      </div>

      {/* Footer row: valid-till + pattern in muted text */}
      <div style={{
        display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap',
        fontSize: 10, color: '#8892a8',
      }}>
        {r.signalBarTs && (
          <span style={{ fontFamily: mono }}>Signal fired at {formatIstTime(r.signalBarTs)}</span>
        )}
        {r.validTillTs && (
          <span style={{ fontFamily: mono }}>· valid till {formatIstTime(r.validTillTs)}</span>
        )}
        {r.sector && <span>· {r.sector}</span>}
      </div>

      <div style={{
        marginTop: 10, fontSize: 11, color: '#2563eb', fontWeight: 600,
      }}>
...
function Line({ label, value, color, strong, muted }) {
  return (
    <div style={{
...
function WatchCard({ r, onTap, category, scheduledChecks }) {
  ...
  return (
    ...
    >
      ...
      {/* Progress bar */}
      <div style={{
        height: 4, borderRadius: 2, background: '#e2e5eb', marginTop: 8, overflow: 'hidden',
      }}>
...
/** Empty-state card rendered when a section has no items. */
...
function SectionHeader({ title, count, color = '#1a1d26' }) {
  return (
    <div style={{
      display: 'flex', alignItems: 'baseline', justifyContent: 'space-between',
...
export default function NoviceModePage({
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <select
              value={novIndex}
              onChange={(e) => setNovIndex(e.target.value)}
              disabled={scanning}
              style={{
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 4, marginLeft: 'auto' }}>
            <input
              ...
              style={{ margin: 0 }}
            />
            <span>Auto-refresh watch list</span>
          </label>
        </div>
      </div>
      ...
      {/* First-use empty state — shown before anything has been scanned */}
      {!scanning && !results.length && !error && (
        <div style={{
