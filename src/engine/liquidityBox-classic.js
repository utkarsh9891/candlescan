/**
 * Multi-day consolidation box detection for swing trading.
 * Wider params than v2: 5-20 day segments, ATR×3.0 threshold.
 */
...
export function detectLiquidityBox(candles) {
  ...
  const winStart = Math.max(0, candles.length - 40); // wider window for daily
  ...
  // 5-20 day segments (multi-day consolidation)
  for (let len = 5; len <= Math.min(20, win.length); len++) {
    for (let i = win.length - len; i >= 0; i--) {
      ...
      // ATR threshold: 3.0× (wider for daily)
      if (boxR >= atr14 * 3.0) continue;
      if (boxR <= 0) continue;
      ...
      const lenScore = Math.log2(Math.max(1, len / 5));
  ...
  const quality = Math.min(1,
    (tightness / 2) * 0.35 + (volScore / 1.5) * 0.35 + (Math.min(1, best.len / 15)) * 0.30
  );
